...
The OrgSoft Web App will be a cloud based application that will allow companies to register with the system to allow for their employees to track and record human resources related information. The system will comprise of a common employee section, which will allow each employee to maintain data such as hours worked specific to them, as well as an HR admin section, which will feature more extensive data access such as employment status. The OrgSoft system architecture shall be comprised of a client-side, browser-based interface (frontend), a cloud-hosted server (backend), and a relational database that will for storing and querying data. The frontend will consist of various web-pages that will allow users to easily input, request, and query information stored in the database. The server backend and database will be configured to optimize concurrent access.
...
- 5.2.1		OrgSoft Frontend CSC -- GUI components that the user will interact with
	- 5.2.1.1		Register Company CSU -- modules for displaying web page allowing the user to register a company in the system
...
- 5.3.1	Frontend
	...
	- 5.3.1.7		The Frontend shall direct users with different levels of access to different main pages
		-5.3.1.7.1  The Frontend will have an admin, supervisor, and employee level of access
	...
	- 5.3.1.12		The Frontend shall react to mouse clicks on displayed buttons
		-5.3.1.12.1 The Frontend will contain buttons for navigating to different pages, submiting inputs, and clocking in / out
	...
	- 5.3.1.15		The Frontend shall have a Timecard page
		...
		- 5.3.1.15.4	The Timecard page shall have buttons for editing the current timecard's recorded times
			- 5.3.1.15.4.1  The Timecard page will only allow supervisors and admins to edit timecards for employees under them
	...
	- 5.3.1.17		The Frontend shall have an Employee Information page
		- 5.3.1.17.1	The Frontend shall have inputs for editing employee information
			-5.3.17.1.1 The Frontend will allow admins to edit name, hire date, and flsa status
...
- 5.3.2	Backend
	...
	- 5.3.2.5		The Backend shall pass error messages with details about connection issues to the Frontend
	...
	- 5.3.2.14		The Backend shall respond with a 404 error code when a page or route is not found
	- 5.3.2.15		The Backend shall respond with a 500 error code when there is an error in the client-side server
	- 5.3.2.16		The Backend shall respond with a 200 error code when an HTTP request is successfully made and returned
	- 5.3.2.17		The Backend shall respond with a 401 error code when an HTTP request is unauthorized for a particular user
...
- 5.3.3	Database
	...
	- 5.3.3.4		The Database shall be able to generate aggregate data that can be used for reports
	- 5.3.3.5		The Database shall implement procedures for backing-up stored data
	- 5.3.3.6		The Database shall be accessible only through the Backend API
	- 5.3.3.7		The Database shall be hosted by a cloud service
	- 5.3.3.8		The Database shall maintain activity tracking logs
	- 5.3.3.9		The Database shall contain a table containing employee account login information
	- 5.3.3.10		The Database shall contain a table containing a list of registered companies
	- 5.3.3.11		The Database shall contain a table containing employee personal information
	- 5.3.3.12		The Database shall contain a table containing employee review information
	- 5.3.3.13		The Database shall contain a table containing employee timecard data
...
The graphical user interface shall follow established interface design guidelines (e.g. OSX Human Interface Guidelines)
